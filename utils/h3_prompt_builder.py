...
Each shot object has description, start_seconds, and dialogues. A dialogue item has literal_index, speaker_key, speaker_description, and delivery; never include its text. If Director starts are supplied, return exactly that many shots and copy those start_seconds values. For Ref2VA, references must contain every allowed label exactly once and in order. Every reference object uses the exact property names label, definition, retention, and retention_detail; never emit reference_label, visible_retention, or audio_retention. The retention property for visible media is one of fully_preserved, partially_preserved, attribute_transfer, weak_reference; the same retention property for Audio is one of fully_copy, partially_copy, reference, weak_reference. task_types uses only keyframe completion, reference generation, video editing, video continuation, audio reuse, audio reference. Base modes must return empty task_types, summary, and references.
