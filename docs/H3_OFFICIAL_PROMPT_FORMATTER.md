...
Known model spelling aliases are normalized narrowly before strict validation: `reference_label` maps to `label`, while `visible_retention` or `audio_retention` maps to the canonical `retention` property according to the registered media family. Canonical and aliased fields with different values are rejected, and all unrelated unknown fields remain errors.
