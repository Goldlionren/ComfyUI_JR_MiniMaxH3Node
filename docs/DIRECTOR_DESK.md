...
Items support selection, drag, left/right resize, 0.1-second snapping, duplicate, midpoint split, delete and ordering actions. The wider highlighted edge handles are the resize targets; dragging the body moves the whole interval. Right-click opens the same contextual operations; double-click focuses the Inspector. Inspector fields now use one item-level draft: **Save** validates and commits role, timing, source range, Direction and Notes together as one undo step, while **Cancel** discards the draft. Clicking another item or starting a timeline drag is blocked while that draft is unsaved, so an edit can never disappear silently. Timeline dragging also uses a local draft and commits once on pointer release.
